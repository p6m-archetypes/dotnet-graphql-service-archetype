--- Render-verification suite for the .NET GraphQL service archetype: each persistence variant lays
...
for _, persistence in ipairs({ "PostgreSQL", "MySQL" }) do
  local label = "dotnet-graphql[" .. persistence .. "]"
...
local none_project = prova.fixture("dotnet-graphql[None]:project", Scope.File, function(ctx)
  return archetect.render{
    source = SRC,
...
prova.group("dotnet-graphql[None]:image", { requires = { "docker" } }, function(g)
...
prova.group("dotnet-graphql[None]:ci", { requires = { "docker" }, tags = { "standards" } }, function(g)
